...
def substitute_expr(expr, var_map):
    """Replace Var nodes with Num constants according to var_map."""
...
def apply_sccp(ir, cfg, ssa_info, sccp_result):
    """Rewrite IR using SCCP constant values."""
    new_ir = list(ir)
    ...
    for block in cfg.nodes():
        ...
        # Build a map of variable -> constant value for this block
        var_map = {}
        ...
        if isinstance(instr, ChironAST.AssignmentCommand):
            ...
            # Check if condition resolves to a constant
...
def apply_adce(ir, cfg, ssa_info, live_blocks, executable_blocks):
    """Replace dead instructions with NoOpCommand."""
    new_ir = list(ir)
...
def optimize(irHandler, args):
    ...
    # Phase 1: M1 local optimizations
    if do_fold or do_simp:
        ir = run_m1_passes(ir, do_fold, do_simp)

    # Phase 2: SSA + SCCP + ADCE
    if do_sccp or do_adce:
        # Build CFG with single-statement blocks
        ...
        if do_sccp:
            ...
            # Apply SCCP results to IR
            ir = apply_sccp(ir, cfg, ssa_info, sccp_result)

            # Run M1 passes again to fold newly created constants
            if do_fold or do_simp:
        ...
        if do_adce:
            if do_sccp:
                ...
                # Need to rebuild CFG and SSA after SCCP rewrites
                cfg = cfgB.buildCFG(ir, "opt_cfg", isSingle=True)
                ssa_info = SSAInfo(cfg)
                ssa_info.build()
