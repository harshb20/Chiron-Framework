"""
SSA Construction for Kachua IR.

Builds analysis-level SSA metadata (side tables) over the existing CFG.
Does NOT rewrite the IR — phi functions and variable versions are metadata only.
"""
...
class SSAInfo:
    ...
    def __init__(self, cfg):
        ...
        # Dominator tree
        self.idom = {}            # block -> immediate dominator block
        self.dom_children = {}    # block -> list of children in dom tree

        # Dominance frontier
        self.dom_frontier = {}    # block -> set of blocks

        # Phi functions: block -> {var: {pred_block: version}}
        self.phi_nodes = {}

        # SSA versioning
        self.var_version_def = {} # (var, block) -> version defined here
        self.var_version_use = {} # (var, block) -> version used here
        self.def_site = {}        # (var, version) -> block where defined
        self.uses = {}            # (var, version) -> set of blocks that use it

        # Variable info
        self.all_vars = set()
        self.def_blocks = {}      # var -> set of blocks that define it (pre-SSA)

        # Block ordering
        self.rpo = []             # reverse post-order
        self._start = None
        self._end = None

    def build(self):
        """Run full SSA construction pipeline."""
    ...
    # -------------------------------------------------------------------
    # Internal: find START and END blocks
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # Internal: collect variable def/use info
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # Internal: Reverse Post-Order via DFS
    # -------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------
    # Internal: Dominator computation (iterative)
    # -------------------------------------------------------------------

    def _compute_dominators(self):
        # Map blocks to RPO indices for efficient idom extraction
        ...
        for b in self.rpo:
            if b == self._start:
                continue
            # idom is the block in dom[b]-{b} with largest RPO index (closest to b)
            candidates = dom[b] - {b}
            if not candidates:
                continue
    ...
    # -------------------------------------------------------------------
    # Internal: Phi Placement (Iterated Dominance Frontier)
    # -------------------------------------------------------------------
        ...
        for var in self.all_vars:
            ...
            while worklist:
                ...
                for b in self.dom_frontier.get(d, set()):
                    if b not in has_phi:
                        has_phi.add(b)
                        self.phi_nodes[b][var] = {}  # pred->version, filled in renaming
                        if b not in ever_on_worklist:
                            ever_on_worklist.add(b)
                            worklist.append(b)

    # -------------------------------------------------------------------
    # Internal: SSA Renaming (dominator tree walk)
    # -------------------------------------------------------------------
    ...
    def _rename_variables(self):
        ...
        def current_version(var):
            ...
            return -1  # input parameter / undefined

        def rename_block(block):
            pushed = {var: 0 for var in self.all_vars}

            # 1. Process phi functions at this block (they define new versions)
            ...
            if instr is not None:
                # Record USES first (read current stack top)
                for var in vars_used_in(instr):
            ...
            for succ in self.cfg.successors(block):
                for var in self.phi_nodes.get(succ, {}):
                    ...
                    # Track that the phi uses this version
                    key = (var, ver)
                    if key not in self.uses:
                        self.uses[key] = set()
